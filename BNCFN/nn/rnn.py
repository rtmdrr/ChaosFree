...
from BNCFN.functional import BNCFNCell as BNCFNCellF
...
class BNCFN(Module):
  ...
  def __init__(
    ...
    for layer in range(num_layers):
      for direction in range(num_directions):
        ...
        w_ih = nn.Parameter(T.Tensor(3 * hidden_size, layer_input_size))
        w_hh = nn.Parameter(T.Tensor(2 * hidden_size, hidden_size))
        if bias:
          b_ih = nn.Parameter(T.Tensor(3 * hidden_size))
          b_hh = nn.Parameter(T.Tensor(2 * hidden_size))
  ...
  def _apply(self, fn):
    ret = super(BNCFN, self)._apply(fn)
    self.flatten_parameters()
    return ret
      ...
      for layer in range(self.num_layers):
        for direction in range(directions):
          if self.bias:
            ...
            hy = BNCFNCellF(os[direction][time], hx[layer][direction], w_ih, w_hh, b_ih, b_hh)
            hx[layer][direction] = hy
  ...
  def __setstate__(self, d):
    super(BNCFN, self).__setstate__(d)
